...
class KnowledgeGraphUtils:
    ...
    def find_merge_candidates(self, label: str, node_type: str = None, semantic_label: str = None, category: str = None, k: int = 5) -> List[Node]:
        ...
        """
        print(f"🔍 FIND_MERGE_CANDIDATES: Looking for '{label}' (type: {node_type}, semantic: {semantic_label})")
        ...
        if label.lower() in well_known_entities:
            print(f"🔍 FIND_MERGE_CANDIDATES: '{label}' is a well-known entity, doing exact match")
            ...
            print(f"🔍 FIND_MERGE_CANDIDATES: Found {len(exact_matches)} exact matches: {[n.label + ' (' + n.node_type + ')' for n in exact_matches]}")
            for node in exact_matches:
                # CRITICAL: Only include candidates with matching node_type
                if node_type is None or node.node_type == node_type:
                    print(f"🔍 FIND_MERGE_CANDIDATES: Adding candidate {node.label} (type: {node.node_type})")
                    candidates[node.id] = node
                else:
                    print(f"🔍 FIND_MERGE_CANDIDATES: Skipping {node.label} (type: {node.node_type}) - type mismatch")
            # Rank and return
            ranked = self._rank_candidates(list(candidates.values()), label_embedding, semantic_label_embedding, category)
            print(f"🔍 FIND_MERGE_CANDIDATES: Returning {len(ranked)} ranked candidates")
            return ranked
                ...
                for existing_edge in candidate_edges:
                    try:
                        ...
                        if similarity >= similarity_threshold:
                            print(f"🔗 Semantic edge duplicate detected!")
                        ...
                        print(f"⚠️ Error computing edge similarity: {e}")
                        continue
