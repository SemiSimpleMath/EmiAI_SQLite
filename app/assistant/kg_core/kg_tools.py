...
def merge_nodes(n1_id, n2_id, session: Session) -> Node:
    ...
    print(f"✅ Merged aliases: {node1.aliases}")
    ...
    for edge in edges:
        ...
        if sid is None or tid is None:
            print(f"⚠️ Skipping corrupt edge {edge.id}")
            continue
        ...
        if new_sid == new_tid:
            print(f"🗑️ Self-loop created: deleting edge {edge.id}")
        ...
        if existing:
            print(f"🗑️ Duplicate edge exists: deleting edge {edge.id}")
            deletions.append(edge.id)
        else:
            print(f"✅ Scheduling reassignment of edge {edge.id}")
...
def describe_node(node_id, session: Session, filters: Dict[str, Any] = None, max_edges: int = 50) -> Dict[str, Any]:
    ...
    if text_filter:
        ...
        all_edge_scores = []
        print(f"🔍 Text filtering: '{text_filter}' - collecting top {k_matches} matches")
        print(f"🔍 Checking {len(inbound)} inbound edges for text matches")
        ...
        for edge in inbound:
            if edge.sentence and edge.sentence_embedding is not None:
                ...
                print(f"🔍 Edge sentence: '{edge.sentence[:50]}...' similarity: {similarity:.3f}")
        ...
        print(f"✅ Selected top {len(inbound_filtered)} inbound edges")
        
        # Collect outbound edges with similarity scores
        print(f"🔍 Checking {len(outbound)} outbound edges for text matches")
        ...
        print(f"✅ Selected top {len(inbound_filtered)} inbound + {len(outbound_filtered)} outbound edges")
        ...
        print(f"🔍 Text filtering results: {len(inbound)} inbound, {len(outbound)} outbound edges")
