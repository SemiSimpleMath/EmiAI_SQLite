...
class TaxonomyOrchestrator:
    ...
    def classify_node(
        ...
        if node_type == "Entity":
            ...
            if label_lower == "jukka":
                ...
                if user_tax:
                    print(f"\n🎯 Classifying: '{label}' ({node_type}) - SYSTEM ENTITY (Jukka)")
                    path = self.manager.get_taxonomy_path(user_tax.id)
                    print(f"   ✅ Hardcoded: {' > '.join(path)} (confidence: 1.00)")
                ...
                if assistant_tax:
                    print(f"\n🎯 Classifying: '{label}' ({node_type}) - SYSTEM ENTITY (Emi)")
        ...
        if branch_selector_agent:
            ...
            try:
                ...
                if isinstance(llm_result, dict) and 'top_paths' in llm_result:
                    ...
                    print(f"   ✅ LLM found {len(llm_candidates)} candidate paths")
                    for i, cand in enumerate(llm_candidates[:3], 1):
                        print(f"      {i}. {cand['path']} (score: {cand['score']:.2f})")
                else:
                    print(f"   ⚠️  LLM returned nothing")
        ...
        if best_paths:
            ...
            if validated_paths:
                ...
                print(f"   ✅ Beam found {len(beam_candidates)} candidate paths")
                for i, cand in enumerate(beam_candidates[:3], 1):
                    print(f"      {i}. {cand['path']} (score: {cand['score']:.2f})")
            else:
                print(f"   ⚠️  No paths passed validation")
        else:
            print(f"   ⚠️  Beam search returned no paths")
        ...
        print(f"\n📋 Returning {len(all_candidates)} candidates to pipeline for critic review")
    ...
    def _extract_type_concepts(self, label: str, sentence: str, node_type: str, agent) -> List[str]:
        ...
        For "Jukka" + "is discussing" → ["person", "user"]
    ...
    def _viterbi_beam_search(
        ...
        print(f"   🎯 Two-phase search with {num_hints} hints: {hints}")
        ...
        for depth in range(MAX_DEPTH):
            ...
            for beam in beams:
                ...
                if depth == 0 and branch_selector_agent and len(children) > 5:
                    ...
                    print(f"   🤖 LLM pruned to {len(children)} branches at first level")
                ...
                if depth == 0:
                    print(f"      Pre-filtered: {children_before} → {len(children)} children")
                ...
                for child in children:
                    ...
                    if depth == 0:
                        ...
                        if best_hint_idx is not None and child_hint_scores[best_hint_idx] > HINT_MATCH_THRESHOLD:
                            print(f"         ✓ {child.label} best → '{hints[best_hint_idx]}' (sim: {child_hint_scores[best_hint_idx]:.3f})")
            ...
            if depth > 2 and beams:
                ...
                if gain < EPSILON_GAIN:
                    print(f"   ⏸️  Early stop at depth {depth+1}: gain {gain:.4f} < {EPSILON_GAIN}")
        ...
        print(f"   🔍 Phase 1 complete: {len(completed_paths)} paths explored")
        
        # === PHASE 2: Global hint assignment ===
        print(f"   🎯 Phase 2: Assigning hints globally...")
        ...
        try:
            ...
            if len(branch_scores) > 1:
                ...
                if top_score > 0.85 and second_score < 0.5:
                    ...
                    print(f"      🎯 Clear winner detected: {branch_scores[0][0]} (rel: {top_score:.2f}) >> others")
    ...
    def _beam_search_OLD(self, query_embeds: List, root_id: int, label: str, session) -> List[Dict]:
        ...
        for depth in range(1, MAX_DEPTH):
            ...
            # Prune beams: keep top-K + α-margin
            next_beams.sort(key=lambda x: x["score"], reverse=True)
            ...
            if len(next_beams) > BEAM_WIDTH:
                ...
                # Also keep any within α-margin of best
                best_score = kept[0]["score"]
                for beam in next_beams[BEAM_WIDTH:]:
                    if beam["score"] >= best_score - ALPHA_MARGIN:
                        kept.append(beam)
            ...
            #     gain = best_new - best_old
            #     if gain < EPSILON_GAIN:
            #         print(f"   ⏸️  Early stop at depth {depth}: gain {gain:.4f} < {EPSILON_GAIN}")
            #         break
    ...
    def _validate_scored_paths(self, paths: List[Dict], query_embeds: List, session) -> List[Dict]:
        ...
        print(f"   ✅ {len(validated)}/{len(paths)} paths passed Viterbi validation")
    ...
    def _llm_verify_path(
        ...
        try:
            ...
            print(f"   🤖 LLM Verifier: rank={chosen_rank}, confidence={confidence:.2f}")
            print(f"      Reasoning: {reasoning[:100]}...")
        ...
        for depth in range(MAX_DEPTH):
            ...
            for node_id, cumulative_score, path in current_nodes:
                ...
                if child_scores:
                    ...
                    print(f"   🔍 Depth {depth}, Top 5 children: {', '.join([f'{label} ({sim:.3f})' for _, sim, label in top_5])}")
                ...
                if top_k:
                    ...
                    print(f"   → Taking top {len(top_k)} children (best: {best_sim:.3f})")
                    
                    if best_sim < EARLY_STOP_THRESHOLD:
                        print(f"   ⚠️  Early stop: best child score {best_sim:.3f} < {EARLY_STOP_THRESHOLD}")
                        all_candidate_paths.append({
                            "path": current_path,
                            "cumulative_score": cumulative_score,
                            "depth": len(current_path),
    ...
    def _critic_compare_methods(
        ...
        try:
            ...
            if llm_option['confidence'] >= beam_option['confidence']:
                print(f"   ⚠️  Critic unclear, using LLM (higher confidence)")
                return (llm_option['taxonomy_id'], llm_option['confidence'], "llm_guided")
            else:
                print(f"   ⚠️  Critic unclear, using Beam (higher confidence)")
