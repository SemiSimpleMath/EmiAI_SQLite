...
class LocationManager:
    ...
    def _save_location(self):
        ...
        try:
            ...
            logger.debug(f"📍 Location data saved")
        except Exception as e:
    ...
    def _build_calendar_entries(self, days_ahead: int = 7) -> List[Dict]:
        ...
        print(f"📍 _build_calendar_entries: Found {len(events)} total calendar events")
        ...
        print(f"📍 _build_calendar_entries: {len(entries)} with locations, {events_without_location} without")
        return entries
    ...
    def build_location_timeline(self, days_ahead: int = 7) -> List[Dict]:
        ...
        print(f"📍 Found {len(all_calendar_events)} total calendar events")
        ...
        #   - Event names (e.g., "Peter's Broadway Arts" → Lake Forest)
        #   - Guidelines (work from home, drop-off/pick-up patterns)
        #   - Time of day patterns
        print("📍 Calling agent to build location timeline...")
    ...
    def update_current_location(self):
        ...
        self._save_location()
        logger.info(f"📍 Current location: {current.get('label')} (source: {current.get('source')})")
    ...
    def refresh(self) -> Dict[str, Any]:
        ...
        """
        print("📍 LocationManager.refresh() called")
        logger.info("📍 Refreshing location data...")
        self.build_location_timeline()
        current = self.update_current_location()
        print(f"📍 Refresh complete. Current location: {current.get('label', 'Unknown')}")
        return current
