...
def _resources_dir() -> Path:
    # This file lives at: app/assistant/physical_status_manager/manager.py
    # resources is assumed to be at: app/resources
...
class PhysicalPipelineManager:
    ...
    def _load_config(self) -> None:
        ...
        state_file = self._config.get("state_resource_file") or "resource_user_physical_status_state.json"
    ...
    def refresh(self) -> None:
        ...
        for stage_id, stage in self._stages.items():
            ...
            if cursor_updates:
                self._merge_into(self.state.setdefault("cursors", {}), cursor_updates)

            if result.cursor_updates:
                self._merge_into(self.state.setdefault("cursors", {}), result.cursor_updates)

            if result.state_updates:
                self._merge_into(self.state, result.state_updates)
