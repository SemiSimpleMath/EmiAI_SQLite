...
class StatusEffect(BaseModel):
    ...
    action: str = Field(
        default="completed",
        description="Action to take: 'completed' records current timestamp"
    )
...
class Suggestion(BaseModel):
    ...
    status_effects: List[StatusEffect] = Field(
        ...
        description="List of status fields to update if accepted. Each has activity_name and action ('completed' for timestamp recording)."
    )
